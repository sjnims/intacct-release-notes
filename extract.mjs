...
const RELEASES = {
  ...
  '2026-R1': { year: '2026', dir: '2026_Release_1', home: '2026-R1-home.htm', preview: true },
  'hidden-gems-2023': { year: '2023', dir: null, home: '2023-year-end-review.htm', standalone: true },
  'hidden-gems-2024': { year: '2024', dir: null, home: '2024-hidden-gems.htm', standalone: true },
  'hidden-gems-2025': { year: '2025', dir: null, home: '2025-hidden-gems.htm', standalone: true },
  '2026-calendar': { year: '2026', dir: null, home: '2026-release-calendar.htm', standalone: true },
};
...
function urlToOutputFile(url, release) {
  ...
  let rel = url.startsWith(base) ? url.slice(base.length) : new URL(url).pathname.split('/').pop();
  ...
  const processed = parts.map((seg) => {
    ...
    const shortPrefix = release.replace(/^20(\d{2})-R/i, '$1r').toLowerCase() + '-';
...
async function extractPage(page, url, turndown) {
  ...
  await page.waitForSelector('.mc-body-content, #mc-main-content, main', { timeout: 10000 });
  ...
  const { title, html } = await page.evaluate(() => {
    ...
    document.querySelectorAll('.mc-miniToc, .mini-toc, [data-mc-toc]').forEach((el) => el.remove());
    ...
    const mainEl = document.querySelector('.mc-body-content') ||
...
async function discover(release) {
  ...
  if (r.standalone) {
    console.log(`${release} is standalone — skipping discover (will extract single page)`);
    return;
  }
  ...
  await page.waitForSelector('.mc-body-content, #mc-main-content, main, .sidenav', { timeout: 10000 });

  // Collect all links from main content AND sidebar navigation
  const links = await page.evaluate(({ base, home }) => {
    const urls = new Set();
    ...
    document.querySelectorAll('.sidenav a[href], nav a[href], .toc a[href]').forEach((a) => {
      urls.add(a.href);
    });
  ...
  writeFileSync(manifestPath(release), JSON.stringify(manifest, null, 2), 'utf-8');
  console.log(`Wrote manifest with ${pages.length} pages → ${manifestPath(release)}`);
...
async function extract(release, { force = false } = {}) {
  ...
  if (r.standalone) {
    ...
    if (existsSync(out) && !force) {
      console.log(`${release}: ${out} already exists (use --force to re-extract)`);
      await browser.close();
      return;
    }
  ...
  if (!existsSync(indexPath) || force) {
    try {
      const { title, markdown } = await extractPage(page, manifest.homeUrl, turndown);
      const frontmatter = [
        '---',
      ...
      writeFileSync(indexPath, `${frontmatter}\n\n# ${title}\n\n${markdown}\n`, 'utf-8');
      success++;
      console.log(`[${success + skipped + failed}/${total}] ✓ index.md`);
    } catch (err) {
      failed++;
      console.error(`[${success + skipped + failed}/${total}] ✗ index.md: ${err.message}`);
    }
    await delay(300);
  } else {
    skipped++;
  ...
  for (const entry of manifest.pages) {
    ...
    if (existsSync(pagePath) && !force) {
      skipped++;
      console.log(`[${success + skipped + failed}/${total}] ⊘ ${entry.outputFile} (exists)`);
      continue;
    }
    ...
    try {
      ...
      writeFileSync(pagePath, `${frontmatter}\n\n# ${title}\n\n${markdown}\n`, 'utf-8');
      success++;
      console.log(`[${success + skipped + failed}/${total}] ✓ ${entry.outputFile}`);
    } catch (err) {
      failed++;
      console.error(`[${success + skipped + failed}/${total}] ✗ ${entry.outputFile}: ${err.message}`);
    }
...
switch (command) {
  ...
  default:
    ...
    console.log('');
    console.log('Usage:');
    console.log('  node extract.mjs discover <release-id>   Discover sub-pages, write manifest');
    console.log('  node extract.mjs extract <release-id>    Extract pages to markdown');
    console.log('  node extract.mjs all [--force]           Discover + extract all releases');
    console.log('');
