...
-   In the **EXTF\_Sachkontobeschriftungen** General Ledger transactions export, the DATEV ID from the vendor and customer records is displayed for offset accounts. Expense accounts still display the expense account number.
    
-   In the **EXTF\_DebKred\_Stamm** merged vendor and customer export, the DATEV ID displays for vendors and customers. If the vendor or customer record does not have an associated DATEV ID, the field does not display any value.
    
-   There's no impact for the **EXTF\_Buchungsstapel** chart of accounts export.
