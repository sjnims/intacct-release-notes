...
We enhanced the validation for timesheets while they're in the *draft* status to ensure all components of a timesheet are accurate before submission.

Previously, to validate the data in a timesheet, you would have to submit the timesheet to Sage Intacct first. Now, timesheets using Sage Intelligent Time (SIT) are validated automatically, even with a *draft* status.
